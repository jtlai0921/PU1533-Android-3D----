...
public class MySurfaceView extends GLSurfaceView
{
	...
	//主繪制資料
	public GameData gdMainDraw=new GameData();
	...
	public MySurfaceView(MainActivity activity)
	{
		...
		dat=new DoActionThread(MySurfaceView.this);//建立執行動作執行緒
		dat.start();//啟動執行緒
		ActionThread at=new ActionThread();
		at.start();
		bgt=new BulletGoThread(bulletAl,this);//建立定時搬移炮彈的執行緒
		bgt.start();//啟動此執行緒
	}
	...
	public boolean onTouchEvent(MotionEvent e)
	{
		...
		switch(action)									//>>>的意思是無號右移
		{
			case MotionEvent.ACTION_DOWN: //主點down 	
				//向Map中記錄一個新點
				if(Bullet.tankeflag2<3&&activity.followtankeflag<3)
				{
			...
			case MotionEvent.ACTION_UP: //主點up
				...
				else
				{
					if(e.getX()>(180+ssr.lucX)*ssr.ratio&&e.getX()<(500+ssr.lucX)*ssr.ratio&&e.getY()>(150+ssr.lucY)*ssr.ratio&&e.getY()<(340+ssr.lucY)*ssr.ratio)
					{
        				...
        				DoActionThread.bx=0;
        				DoActionThread.by=0;
        				DoActionThread.scale_x=0;
        				DoActionThread.scale_y=0;
        				
        				synchronized(gdMain.dataLock) 	//將主資料鎖上
    					{
        					gdMain.maintkwz[0]=0;
        					gdMain.maintkwz[1]=-100;
    						gdMain.maintkwz[2]=0;
        					gdMain.maintkwz[3]=0;
    						gdMain.maintkwz[4]=0;
    						gdMain.maintkfp[0]=0;
    						...
    						{
    							gdMainDraw.maintkwz[0]=0;
    							gdMainDraw.maintkwz[1]=-100;
    							gdMainDraw.maintkwz[2]=0;
    							gdMainDraw.maintkwz[3]=0;
    							gdMainDraw.maintkwz[4]=0;
    							gdMainDraw.maintkfp[0]=0;
    						}
    					}
        				synchronized(activity.gdFollowDraw.dataLock)	//鎖上繪制資料
        				{
        					activity.gdFollowDraw.followtkwz[0]=0;
        					activity.gdFollowDraw.followtkwz[1]=100;
        					activity.gdFollowDraw.followtkwz[2]=0;
        					activity.gdFollowDraw.followtkwz[3]=0;
        					activity.gdFollowDraw.followtkwz[4]=0;
        					activity.gdFollowDraw.followtkfp[0]=0;
        				}
        				Bullet.anmiStart=false;
        				activity.followanmiStart=false;
						hm.clear();
						bulletAl.clear();
						gdMainDraw.mainTailAl.clear();
						gdMain.mainTailAl.clear();
					}	
	...
	private class SceneRenderer implements GLSurfaceView.Renderer
	{
		...
		public void onDrawFrame(GL10 gl) {
            ...
            {
            	maintkwzTemp[0]=gdMainDraw.maintkwz[0];
            	maintkwzTemp[1]=gdMainDraw.maintkwz[1];
            	maintkwzTemp[2]=gdMainDraw.maintkwz[2];
            	maintkwzTemp[3]=gdMainDraw.maintkwz[3];
            	maintkwzTemp[4]=gdMainDraw.maintkwz[4];
            }
            float followtkwzTemp[]=new float[5];
            synchronized(activity.gdFollowDraw.dataLock)	//鎖上繪制資料
            {
            	followtkwzTemp[0]=activity.gdFollowDraw.followtkwz[0];
            	followtkwzTemp[1]=activity.gdFollowDraw.followtkwz[1];
            	followtkwzTemp[2]=activity.gdFollowDraw.followtkwz[2];
            	followtkwzTemp[3]=activity.gdFollowDraw.followtkwz[3];
            	followtkwzTemp[4]=activity.gdFollowDraw.followtkwz[4];
            }
            
            float maintkfpTemp[]=new float[2];
            synchronized(gdMainDraw.dataLock)	//鎖上繪制資料
            {
            	maintkfpTemp[0]=gdMainDraw.maintkfp[0];
            	maintkfpTemp[1]=gdMainDraw.maintkfp[1];
            }
            if(maintkfpTemp[0]==1)
            {
                	fire();
            ...
            synchronized(activity.gdFollowDraw.dataLock)	//鎖上繪制資料
            {
            	followtkfpTemp[0]=activity.gdFollowDraw.followtkfp[0];
            	followtkfpTemp[1]=activity.gdFollowDraw.followtkfp[1];
            }
            ...
            synchronized(gdMainDraw.dataLock)	//鎖上繪制資料
            {
            	System.out.println(gdMainDraw.mainbulletAl.size());
            	if(gdMainDraw.mainbulletAl!=null)
            	{
            		for(int i=0;i<gdMainDraw.mainbulletAl.size();i++)
                	{
            			mainbulletTemp=gdMainDraw.mainbulletAl.get(i);
            ...
            synchronized(activity.gdFollowDraw.dataLock)	//鎖上繪制資料
            {
            	if(activity.gdFollowDraw.followbulletAl!=null)
            	{
            		for(int i=0;i<activity.gdFollowDraw.followbulletAl.size();i++)
            		{
            			followbulletTemp=activity.gdFollowDraw.followbulletAl.get(i);
            		...
            		if(activity.gdFollowDraw.followbulletAl.size()==1)
            		{
            			activity.gdFollowDraw.followbulletAl.remove(0);
            		}
            	}
            }
           ...
           //繪制主方尾巴
	        ...
	        synchronized(gdMainDraw.dataLock)	//鎖上繪制資料
	        {
	        	if(gdMainDraw.mainTailAl!=null)
		       	{
		       		for(int i=0;i<gdMainDraw.mainTailAl.size();i++)
		       		{
		       			TempData=gdMainDraw.mainTailAl.get(i);
	        ...
	        synchronized(activity.gdFollowDraw.dataLock)	//鎖上繪制資料
	        {
	        	if(activity.gdFollowDraw.followTailAl!=null)
	        	{
	        		for(int i=0;i<activity.gdFollowDraw.followTailAl.size();i++)
	        		{
	        			followTempData=activity.gdFollowDraw.followTailAl.get(i);
	        		...
	        		if(activity.gdFollowDraw.followTailAl.size()==1)
	        		{
	        			activity.gdFollowDraw.followTailAl.remove(0);
	        		}
	        	}
	        }
          ...
          //繪制主方坦克炮彈爆炸紋理
            if(Bullet.anmiStart)
            {
   			 if(Bullet.anmiIndex<trExplo.length-1)
   		    	{//動畫沒有播放完動畫換框
   				Bullet.anmiIndex=Bullet.anmiIndex+1;
   		    	}
   		 	}          
            //繪制爆炸紋理
            if(Bullet.anmiStart)
	    	{//動畫沒有播放完動畫換框
            	gl.glPushMatrix();//保護現場
				gl.glTranslatef(Bullet.anmiX,Bullet.anmiY , -84);//搬移到指定位置
				trExplo[Bullet.anmiIndex].drawSelf(gl);//繪制爆炸動畫目前框
				gl.glPopMatrix();
	    	}
...
//            float[] positionParamsGreen={0,0,90,1};//最後的0表示是定位光
            ...
            //坦克生命紋理
            tanklifeId=initTexture(gl,PicLoadUtil.loadBM(getResources(), "tanklife1.png")); 
            tanklifeId_1=initTexture(gl,PicLoadUtil.loadBM(getResources(), "tanklife.png")); 
            //起始化子彈紋理
	...
	public void fire()
	 {
		...
		float startZ;
		 synchronized(gdMainDraw.dataLock)	//鎖上繪制資料
		 {
			  startAngle=-gdMainDraw.maintkfp[1];
			  startX=gdMainDraw.maintkwz[0]+(float) (10*Math.sin(Math.toRadians(startAngle)));
			  startY=gdMainDraw.maintkwz[1]+(float) (10*Math.cos(Math.toRadians(startAngle)));
		 }
		 startZ=-84;
		 float vx=(float) (BULLET_V*Math.sin(Math.toRadians(startAngle)));
		 float vy=(float) (BULLET_V*Math.cos(Math.toRadians(startAngle)));
		 float vz=0;
		 Bullet bl= new Bullet(mRenderer.bullet,startX,startY,startZ,vx,vy,vz,mRenderer.trExplo,startAngle,this,mainbulletflag);
		 bulletAl.add(
				 bl
				 ); 
	 }
