...
{	
    ...
    public TextureRectDefen(float width,float height,MyGLSurfaceView mv)
    {    	
    	//起始化頂點座標資料的方法
    ...
    {
    	//載入頂點著色器的指令稿內容       
      String  mVertexShader=ShaderUtil.loadFromAssetsFile("vertex_defen.sh", mv.getResources());
        ShaderUtil.checkGlError("==ss==");   
        //載入片元著色器的指令稿內容
      String  mFragmentShader=ShaderUtil.loadFromAssetsFile("frag_defen.sh", mv.getResources());  
        //基於頂點著色器與片元著色器建立程式
        ShaderUtil.checkGlError("==ss==");      
    ...
    { 
         ...
         GLES20.glUniform1f(muAlphaHandle, Constant.denfenAlpha);
         //傳入頂點位置資料
