...
public class Texture_Yuanzhu 
{
	...
	public Texture_Yuanzhu(GameView gameview,float radius,float deepth)
	{
		this.gameview=gameview;
		this.deepth=deepth;
		this.r=From2DTo3DUtil.k2d_3d(radius);
    	this.length=From2DTo3DUtil.k2d_3d(deepth);
    	this.aspan=15;
        ...
        for(float tempY=length/2;tempY>-length/2;tempY=tempY-lspan)//垂直方向lspan長度一份
        {
        	for(float hAngle=360;hAngle>0;hAngle=hAngle-aspan)//水平方向angleSpan度一份
        	{
        		...
        		float y1=(float)(r*Math.sin(Math.toRadians(hAngle)));
        		float z1=tempY;
        		
        		float x2=(float)(r*Math.cos(Math.toRadians(hAngle)));
        		float y2=(float)(r*Math.sin(Math.toRadians(hAngle)));
        		float z2=tempY-lspan;
        		
        		float x3=(float)(r*Math.cos(Math.toRadians(hAngle-aspan)));
        		float y3=(float)(r*Math.sin(Math.toRadians(hAngle-aspan)));
        		float z3=tempY-lspan;
        		
        		float x4=(float)(r*Math.cos(Math.toRadians(hAngle-aspan)));
        		float y4=(float)(r*Math.sin(Math.toRadians(hAngle-aspan)));
        		float z4=tempY;   
	...
	public void drawSelf(GL10 gl,float x,float y,float angle,int id)
    {
        //容許使用頂點陣列
